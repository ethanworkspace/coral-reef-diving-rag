...
(() => {
  ...
  function buildUrl(siteId, radius, offset) {
    ...
    if (!isAllowedRadius(radius)) {
      throw new RangeError("radius must be one of the displayed choices");
    }
    ...
    return `/api/dive-sites/${encodeURIComponent(siteId)}/nearby-edna?${parameters.toString()}`;
  }
  ...
  globalThis.NearbyEdnaQuery = api;
