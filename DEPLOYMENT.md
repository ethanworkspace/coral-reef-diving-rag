...
第一次啟動時，容器會下載海保署保護區邊界、OGL eDNA 與 CWA 公開波流模型資料，建立空間／生物觀測資料庫。它不下載 CMAS 手冊、使用者提供資料或 Reef Check 資料。
...
- `M-B0078-001` 的 REST 呼叫在 2026-09-18 回傳 HTTP 404；部署版改用 CWA 公開模型 JSON，仍須依其發布與有效時間判定是否資料不足。
